...
// Check if a stock symbol supports options trading
...
export function getOptionsSupportedStocks(searchTerm = null) {
  let stocks = optionsStockList;

  if (searchTerm) {
    const term = searchTerm.toLowerCase();
    stocks = optionsStockList.filter(
      (stock) =>
...
export function getStockDetails(symbol) {
  ...
  return {
    found: false,
    message: `Stock with symbol ${symbol} not found in options list`,
  };
}
